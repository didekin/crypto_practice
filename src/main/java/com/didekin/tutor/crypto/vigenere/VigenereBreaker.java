...
import com.didekin.tutor.crypto.charhelper.CharManipulator;
...
import static com.didekin.tutor.crypto.charhelper.CharManipulator.modBase;
import static com.didekin.tutor.crypto.charhelper.CharManipulator.xorCharArrayWithKeyArray;
import static com.didekin.tutor.crypto.vigenere.EnglishLetter.getLetterProbFromOrder;
...
class VigenereBreaker {
    ...
    {
        List<Character> lettersList = getAsciiDecimalCharListFromHexCharsSeq(fullCipheredText);
        List<Character> selectedList = new ArrayList<>(lettersList.size() / period);
        for (int i = 0; i < lettersList.size(); ++i) {
            if (modBase(i, period) == 0) {
    ...
    static List<Character> composeIthStream(String hexSourceText, int ithKeyIndex, int keyPeriod)
    {
        ...
        List<Character> charList = getAsciiDecimalCharListFromHexCharsSeq(hexSourceText);
    ...
    static int getIthCharInKey(List<Character> ithStreamChars)
    {
        ...
        Long[] freqInDecipher = new Long[numberEnglishLetters];
        ...
        for (int i = 0; i < 256; ++i) {
            ...
            charIntStreamArr = stream(xorCharArrayWithKeyArray(asciiIntArrayText, asciiCharArr)).toArray();
            ...
            for (int k = 0; k < freqInDecipher.length; ++k) {
                crossProduct += getLetterProbFromOrder(k) * (freqInDecipher[k] / totalLettersFreq);
            }
